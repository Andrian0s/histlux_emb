"""
Script used to extend paraphrase-mpnet-multilingual-base-v2 to also include Luxembourgish. Data loading modification adaptation.

By: Andrianos Michail for the work titled: Adapting Multilingual Embedding Models to Historical Luxembourgish

Resulting model is publically available at: https://huggingface.co/impresso-project/histlux-paraphrase-multilingual-mpnet-base-v2

Original Pydoc of template script:
...
teacher_model_name = "sentence-transformers/paraphrase-mpnet-base-v2"
...
student_max_seq_length = (
    128  # Student model max. lengths for inputs (number of word pieces)
)
train_batch_size = 32  # Batch size for training
inference_batch_size = 32  # Batch size at inference
max_sentences_per_language = 21000  # Maximum number of  parallel sentences for training

num_train_epochs = 2  # Train for x epochs
...
max_sentences_per_dataset = 21000
dataset_to_use = ["fredxlpy/LuxAlign", "Andrianos/HistLuxAlign"]
total_training_data = 40000
# dataset_to_use = "fredxlpy/LuxAlign"
...
for source_lang in source_languages:
    for target_lang in target_languages:
        ...
        try:
            if isinstance(dataset_to_use, list):
                train_datasets = []

                for dataset in dataset_to_use:
                    try:
                        train_dataset = load_dataset(dataset, subset, split="train")
                        train_dataset = train_dataset.select(
                            range(min(len(train_dataset), max_sentences_per_dataset))
                        )
                        train_datasets.append(train_dataset)
                    except Exception as exc:
                        logging.error(
                            f"Could not load dataset {dataset}/{subset}: {exc}"
                        )

                # Merge datasets while keeping the dataset format
                combined_datasets = (
                    concatenate_datasets(train_datasets) if train_datasets else None
                )

            else:
                combined_datasets = load_dataset(dataset_to_use, subset, split="train")
                combined_datasets = combined_datasets.select(
                    range(min(len(combined_datasets), max_sentences_per_dataset))
                )

        except Exception as exc:
            logging.error(f"Could not load dataset(s) {dataset_to_use}: {exc}")
        ...
        try:
            ...
            logging.info(
                f"Could not load dataset {dataset_to_use}/{target_lang}-{source_lang} dev split, splitting 1k samples from train"
            )
            dataset = combined_datasets.train_test_split(
                test_size=int(len(combined_datasets) - total_training_data),
                shuffle=True,
            )
...
def prepare_dataset(batch):
    return {
        "english": batch["en"],
        "non_english": batch["lb"],
        "label": teacher_model.encode(
            batch["en"], batch_size=inference_batch_size, show_progress_bar=False
        ),
    }
...
for subset, eval_dataset in eval_dataset_dict.items():
    ...
    dev_mse = MSEEvaluator(
        source_sentences=eval_dataset["en"],
        target_sentences=eval_dataset["lb"],
        name=subset,
...
eval_dataset_dict = eval_dataset_dict.map(
    prepare_dataset, batched=True, batch_size=30000, remove_columns=column_names
)
...
model_name = (
    student_model_name
    if "/" not in student_model_name
    else student_model_name.split("/")[-1]
)
try:
    student_model.push_to_hub(
        f"{model_name}-multilingual-{'-'.join(source_languages)}-{'-'.join(target_languages)}"
    )
except Exception:
    logging.error(
